function Settings({ handleLink, setLink, link, links, handleDeleteLink }) {
  return (
    <div>
      ...
      <h2 className="text-xl font-bold ">Allowed URL's</h2>
      <div className=" flex flex-col items-center h-[17rem] ">
        <div className="overflow-y-auto overflow-x-hidden w-full ">
          {links.map((link) => {
            return (
              <div
                className="  flex items-center justify-between rounded-lg  my-2 bg-[#D8EFD3]  w-[70%] m-auto p-2  "
                key={link.id}
              >
                <span className=" mx-2 text-lg  w-[10rem]">{link.link}</span>
